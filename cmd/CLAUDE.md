...
- `disco check --rules ./policies --severity high --exit-nonzero` — Runs OPA Rego policies against store. `--rules` takes `.rego` files or directories (recursive). Engine in `internal/policy/` ships in OSS; no first-party policies bundled — bring your own (Conftest AWS, regula, in-house CIS pack). Curated compliance packs (NIST, CIS, PCI-DSS, Well-Architected) are paid add-ons. Each policy module must populate `data.disco.deny` (set) with finding objects shaped `{id, severity, message, resource_id?, tags?, category?, remediation?, ref_url?}`. Input shape: `{id, provider, account_id, type, native_id, name, region, status, attributes}` — `attributes` is the decoded `AttributesJSON` (object), not the raw string.
