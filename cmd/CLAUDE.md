...
- `disco coverage services` — per-service type matrix. Reads scanner-declared `emits []coverage.TypeDecl` and matches against live upstream registries (CFN ListTypes / ARM Providers/List / GCP Discovery). Buckets (`--filter`): covered / uncovered / not-scannable / uncatalogued / upstream-missing, plus the `duplicate` cross-cut. `uncatalogued` = real SDK-scanned resource no registry lists (kms grant, Detective/Inspector members, Entra, Azure SQL/network proxy children) — does not trip `--check-strict`. `not-scannable` = upstream-listed type a provider deliberately does not scan (CFN sub-resource with no List API, ephemeral task/report record, preview service with no SDK) — declared via `coverage.Skipper` (`internal/providers/aws/aws_skips.go`), carries a `reason` column, also does not trip `--check-strict`. `--filter duplicate` is not a bucket but an audit view of the cross-catalog SR↔CFN twins that `coverage.CanonicalKeyer` collapses into `covered` (covered rows carrying a `"duplicate of …"` reason) — see `internal/coverage/CLAUDE.md`. (No synthetic bucket: cross-tenant foreign account/sub/project refs are real self-node types, not fabricated stubs.) A failed upstream fetch (e.g. expired/missing credentials) is **always fatal** — it returns `errCoverageRegistryUnreachable` → exit 2 regardless of `--check-strict`, before any matrix is rendered, so an empty registry never masquerades as fleet-wide upstream-missing drift. `--check-strict` only governs genuine drift: exit 1 on any upstream-missing row (the distinct exit code lets CI branch transient/credential failures from real drift). `--regions us-east-1,us-west-2` triggers per-region CFN calls + unions the upstream slice (catches per-region launch drift).
...
- **Scan progress (`cmd/progress.go`).** Permanent per-service / resolve lines stream through `progress.line()` to stderr; on an interactive terminal a transient bottom line animates a braille spinner (`⠋ scanning… <elapsed> · <n> done`) so a slow single service or the resolve phase still shows liveness. No `done/total` fraction — the denominator (services × scopes) isn't knowable up front (Azure subscriptions / GCP projects are discovered at scan time), so the spinner is an honest elapsed + completed-unit-count indicator, not a fake %. The earlier `#N` per-line counter was removed (low signal); the count now lives only in the spinner. The spinner is gated `!quiet && !no-progress && isTerminal(stderr)`, where `isTerminal` is the stdlib `*os.File` + `os.ModeCharDevice` check (no `x/term`/`go-isatty` dep). Off-TTY (CI, redirected stderr, tests with a `*bytes.Buffer`) the ticker never starts and `line()` is a plain newline write — identical to no spinner, so CI logs stay clean/greppable. Clears use `\r` + space-overwrite (no ANSI → portable to non-VT Windows consoles). All stderr writes funnel through one mutex so the ticker goroutine and the concurrent `OnServiceComplete` callers never garble. `p.stop()` (idempotent, `sync.Once`) runs after `RunScanners` and before any other stderr writer (warnings/errors). Two switches: `--quiet` = only the final summary; `--no-progress` = keep the per-service lines, drop the animation.
