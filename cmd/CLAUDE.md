...
- `disco coverage services` — per-service type matrix. Reads scanner-declared `emits []coverage.TypeDecl` and matches against live upstream registries (CFN ListTypes / ARM Providers/List / GCP Discovery). Buckets: covered / uncovered / synthetic / upstream-missing. A failed upstream fetch (e.g. expired/missing credentials) is **always fatal** — it returns `errCoverageRegistryUnreachable` → exit 2 regardless of `--check-strict`, before any matrix is rendered, so an empty registry never masquerades as fleet-wide upstream-missing drift. `--check-strict` only governs genuine drift: exit 1 on any upstream-missing row (the distinct exit code lets CI branch transient/credential failures from real drift). `--regions us-east-1,us-west-2` triggers per-region CFN calls + unions the upstream slice (catches per-region launch drift).
