...
15. **Compute surfaces** — Elastic Beanstalk. *(App Runner + Batch + Lightsail landed; Lightsail = inventory-only (no cross-service resolvers). Elastic Beanstalk = legacy CFN-stack-orchestrating service, complex; deferred. See COMPLETED R2.15 entries.)*
