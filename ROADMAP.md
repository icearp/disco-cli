...
### R2.18 Neptune (audit-only — already covered by RDS scanner, this session)
- **Neptune** uses the shared RDS control-plane API (verified via `rds:CreateDBCluster.Engine` SDK doc: valid values include `aurora-mysql`, `aurora-postgresql`, `mysql`, `postgres`, **`neptune`**). Existing `scanRDSClusters` (`rds_scanners.go:142`) and `scanRDSInstances` (`rds_scanners.go:115`) call `Describe*` paginators without an `Engine` filter, so Neptune clusters/instances already persist as `aws:rds:cluster` / `aws:rds:instance` rows. The `Engine` field on each row's `AttributesJSON` distinguishes them (`Engine=neptune`); existing RDS resolvers (cluster→KMS, instance→KMS, etc.) apply uniformly. Semantic mismatch (auditors might expect `aws:neptune:cluster` type) deemed acceptable — engine field is queryable and mirrors AWS's own API design choice (Neptune chose to ride on RDS, DocumentDB did not). Audit-move per CLAUDE.md "ROADMAP vs source drift" rule.
