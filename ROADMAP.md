...
14. **Governance surface** — Artifact, Control Tower. Group; prioritise by user demand. *(Service Catalog + Audit Manager landed; sub-resources deferred within each — see COMPLETED R2.14 entries. Artifact = S3-backed compliance reports, narrow graph value. Control Tower = small read-only landing-zone status API.)*
