...
### R2.18 Neptune + DocumentDB (audit-moved — already covered by RDS scanner, this session)
- **Neptune** + **DocumentDB** both ride on the shared RDS control-plane API. Existing `scanRDSInstances` (`rds_scanners.go:115`) calls `rds.NewDescribeDBInstancesPaginator` with no `Engine` filter; existing `scanRDSClusters` (`rds_scanners.go:142`) likewise calls `rds.NewDescribeDBClustersPaginator` unfiltered. Both APIs return rows for all engines (`aurora-mysql`, `aurora-postgresql`, `mysql`, `postgres`, `oracle-*`, `sqlserver-*`, **`neptune`**, **`docdb`**), so Neptune and DocumentDB clusters/instances already persist as `aws:rds:cluster` / `aws:rds:instance` rows. The `Engine` field on each row's `AttributesJSON` distinguishes them; existing RDS resolvers (cluster→KMS, instance→KMS, instance→OptionGroup, cluster→param-group, etc.) apply uniformly. Functionally covered; semantic mismatch (some auditors might expect `aws:neptune:cluster` / `aws:docdb:cluster` types) deemed acceptable — the engine field is queryable and mirrors AWS's own API design choice. Future split into dedicated types would be cosmetic, not functional. Audit-move per CLAUDE.md "ROADMAP vs source drift" rule.
