...
// in aws_types.go — a Type* identifier declared but referenced nowhere
// else in the package. Without this guard, retired services leave dead
// constants behind that silently expand the package vocabulary and hide
// the genuine catalog inside noise.
//
...
func TestEveryTypeConstantIsUsed(t *testing.T) {
	declared := parseTypeConstants(t, "aws_types.go")
	used := collectIdentRefs(t, ".", "aws_types.go")
	...
	if len(orphans) > 0 {
		t.Errorf("orphan disco-type constants in aws_types.go (declared but referenced nowhere else in the package):\n  %s",
