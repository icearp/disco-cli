...
func scanEC2TrafficMirror(ctx context.Context, client *ec2.Client, acct *account, region string, st *store.Store, scanID string) (total, inserted int, err error) {
	...
	g.Go(func() error {
		tt, nn, e := scanTrafficMirrorFilters(ctx, client, acct, region, st, scanID)
		add(tt, nn)
		return e
	})
	g.Go(func() error {
		tt, nn, e := scanTrafficMirrorFilterRules(ctx, client, acct, region, st, scanID)
		add(tt, nn)
		return e
	})
	g.Go(func() error {
		tt, nn, e := scanTrafficMirrorSessions(ctx, client, acct, region, st, scanID)
		add(tt, nn)
		return e
	})
	g.Go(func() error {
		tt, nn, e := scanTrafficMirrorTargets(ctx, client, acct, region, st, scanID)
		add(tt, nn)
		return e
	})
	err = g.Wait()
