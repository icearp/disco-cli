...
//
// disco builds a fresh SDK client per service/phase (180+ scanners), and each
// client gets its own azcore BearerTokenPolicy with an empty token cache. The
// policy's first request therefore calls the underlying credential's GetToken,
// and DefaultAzureCredential.GetToken serializes under concurrency (~300ms
// each). With the scan firing dozens of clients at once that token acquisition —
// not the network or ARM — was the dominant scan-time cost (measured ~18s for 60
// concurrent fresh clients vs ~0.8s through this cache). Wrapping the credential
// once and handing the wrapper to every client turns each policy's first call
// into an instant cache hit.
...
// GetToken returns a cached token for the requested scopes when one is present
// and not within five minutes of expiry; otherwise it fetches from inner and
// caches the result. The five-minute skew matches azcore's own refresh window
// so callers never see a token that the SDK would consider stale.
