...
// rule: every disco type mirrors its upstream ARM resource-type key. So
// AlgorithmicKey (strip kebab dashes, turn the ':' sub-resource separators back
// into the ARM '/' hierarchy) must reconstruct one of the ARM keys the type is
// registered under. A new Azure type whose string drifts from upstream — an
// added hyphen the ARM key lacks, a '/' instead of ':', a wrong singular/plural
// — fails here. Where one disco type intentionally collapses several ARM
// resource types (DNS record-sets), reconstruction can't match a single key, so
// those are skipped.
