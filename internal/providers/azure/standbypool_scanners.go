...
func scanStandbyPool(ctx context.Context, sub *subscription, cred *azidentity.DefaultAzureCredential, st *store.Store, scanID string) (total, inserted int, err error) {
	...
	vmTotal, vmIns, vmErr := azSimpleScan(ctx, "armstandbypool:StandbyVirtualMachinePools.ListBySubscription", TypeStandbyVMPool, sub, st, scanID,
	...
	if err != nil {
		return vmTotal, vmIns, fmt.Errorf("armstandbypool:NewStandbyContainerGroupPoolsClient: %w", err)
	}
	cgTotal, cgIns, cgErr := azSimpleScan(ctx, "armstandbypool:StandbyContainerGroupPools.ListBySubscription", TypeStandbyContainerGroupPool, sub, st, scanID,
	...
	return vmTotal + cgTotal, vmIns + cgIns, err
}
