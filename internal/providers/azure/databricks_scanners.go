...
func scanDatabricks(ctx context.Context, sub *subscription, cred *azidentity.DefaultAzureCredential, st *store.Store, scanID string) (total, inserted int, err error) {
	...
	total, inserted, err = azSimpleScan(ctx, "armdatabricks:Workspaces.ListBySubscription", TypeDatabricksWorkspace, sub, st, scanID,
	...
	if err != nil {
		return total, inserted, fmt.Errorf("armdatabricks:NewAccessConnectorsClient: %w", err)
	}
	at, ai, err := azSimpleScan(ctx, "armdatabricks:AccessConnectors.ListBySubscription", TypeDatabricksAccessConnector, sub, st, scanID,
