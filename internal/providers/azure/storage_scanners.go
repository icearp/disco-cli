...
func scanStorage(ctx context.Context, sub *subscription, cred *azidentity.DefaultAzureCredential, st *store.Store, scanID string) (total, inserted int, err error) {
	...
	total, inserted, err = azSimpleScan(ctx, "armstorage:Accounts.List", TypeStorageStorageAccount, sub, st, scanID,
	...
	if err != nil {
		return total, inserted, fmt.Errorf("armstorageactions:NewStorageTasksClient: %w", err)
	}
	tt, ti, err := azSimpleScan(ctx, "armstorageactions:StorageTasks.ListBySubscription", TypeStorageStorageTask, sub, st, scanID,
