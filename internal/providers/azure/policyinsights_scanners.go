...
func scanPolicyInsights(ctx context.Context, sub *subscription, cred *azidentity.DefaultAzureCredential, st *store.Store, scanID string) (total, inserted int, err error) {
	...
	rTotal, rIns, rErr := azSimpleScan(ctx, "armpolicyinsights:Remediations.ListForSubscription", TypePolicyInsightsRemediation, sub, st, scanID,
	...
	if err != nil {
		return rTotal, rIns, fmt.Errorf("armpolicyinsights:NewAttestationsClient: %w", err)
	}
	aTotal, aIns, aErr := azSimpleScan(ctx, "armpolicyinsights:Attestations.ListForSubscription", TypePolicyInsightsAttestation, sub, st, scanID,
	...
	return rTotal + aTotal, rIns + aIns, err
}
